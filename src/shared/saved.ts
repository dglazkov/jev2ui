...
// that has moved on makes another. Whether others may open it is beside the app, and its owner's to change.
...
export const SAVED_SCREEN = z.object({
  ...
  request: z.object({ prompt: z.string().max(4000), journey: z.record(z.unknown()).optional(), fresh: z.boolean().optional() }),
});
...
export const SAVED_APP = z.object({
  ...
  design: z.object({ choice: z.string().max(20), markdown: z.string().max(1 << 20), seed: z.number(), report: z.record(z.unknown()).optional() }),
