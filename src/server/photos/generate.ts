...
export function makePhoto(brief: Brief): Promise<Photo> {
  const id = createHash("sha256").update(`${IMAGE_MODEL}|${brief.subject}|${brief.of.toLowerCase()}|${brief.ratio}`).digest("hex").slice(0, 16);
...
async function make(id: string, brief: Brief): Promise<Photo> {
  ...
  try {
    ...
    const photo: Photo = { id, alt: brief.of, words: tokens(brief.of), subjects: [brief.subject] };
