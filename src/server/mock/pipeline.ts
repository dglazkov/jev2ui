...
export function runMock(prompt: string, markdown?: string): AsyncGenerator<PipelineEvent> {
  ...
  return run.drive(async () => {
    ...
    const hooks: Partial<Record<Part, PartHooks>> = {
      ...
      groups: { onValue: asTheyComplete((group, g) => refine("groups", refineGroup(run, prompt, g, group, plan!.icons))) },
    ...
    const voice = markdown ? parseDesign(markdown).voice : "";
    const write = (part: Part, agreeWith?: unknown) => {
      const own = hooks[part];
      return streams.write(
        part,
        { system: SYSTEM_PROMPT, prompt: partPrompt(prompt, part, plan ?? null, voice, agreeWith), schema: partSchema(part, plan ?? null) },
    ...
    write("header");
    const designing = loadDesign(markdown ? { markdown } : { brief: prompt });
    const planned = await run.askJev("Jev: plan the screen", { screen: prompt }, planQuestions());
    ...
    const parts: Part[] = ["header", ...(plan.topLevel ? (["nav"] as Part[]) : []), ...plan.blocks.filter((b): b is Exclude<typeof b, "hero"> => b !== "hero")];
