...
test("a feed can render the existing hero when selected, while a plain feed stays a list", () => {
  ...
  const look = { contained: true, icons: true, symbol: featured.symbol };
  const parts = featured.blocks.map((block) => treeOf(KIT_PATTERNS, SCREEN, partNode(block), back, look));
  const tree = [...frameOf(KIT_PATTERNS, SCREEN, back, look, parts.map((tree, i) => ({ name: featured.blocks[i], root: tree[0].id })))!, ...parts.flat()];
