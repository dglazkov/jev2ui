...
export class AppMapDrawer extends LitElement {
  ...
  render() {
    ...
    return html`<header><h3>App map</h3><button class="close" aria-label="Collapse app map" @click=${() => this.send("map-close")}>›</button></header>
      <p class="summary" aria-live="polite">${state ? `${this.made.length} of ${nodes.length} screens made · ${state.status === "reviewing" ? "Reviewing journeys…" : "Revision " + state.revision}` : this.error ? "App map unavailable" : "Planning your app…"}</p>
      ...
      <button class="open" ?disabled=${this.busy || state.status === "reviewing" || chosen.id === this.current} @click=${() => this.send("map-open", chosen.id)}>${chosen.id === this.current ? "Showing on canvas" : this.working.includes(chosen.id) ? "View progress" : this.made.includes(chosen.id) ? "Open screen" : "Generate screen"}</button>
