...
// a visit before signing in and the sign-in after it are the same person's; each page load is a visit. Who is
// signed in is not said here but proven, with the same token every other request carries, and the server reads
// it off. Sending is best effort: an action that does not arrive is lost, and nobody is told.
//
...
/** Who the page thinks is there, said with every action: session.ts says. */
