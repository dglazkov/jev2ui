...
A grammar = questions for a decision model (yes/no, choice, score) + how answers are read + what each part is made of + which catalog pattern draws it + where unwritten values come from + what is decided once words exist. `grammar/screen.md` is production: the tool reads it on every run. `grammar/examples/email.md` is a second graph with no code behind it. `grammar/kit.md` is the catalog (patterns, slots, knobs, sources); `grammar/ios/screen.md` is the iOS idiom's own grammar (hand-owned, written from the HIG: kinds = presentations; asks `sheet`, `editable`, `bar_action`; frame knobs `presentation`, `title`, `leading`, `trailing`), drawn by `grammar/ios/catalog.md` (generated from `patterns-ios.ts`); `grammar/windows/screen.md` is Windows 11's (shares no parts, questions, patterns or components with the others: silhouette asked once per app, `page_type` = Microsoft's page patterns, WinUI parts), drawn by `grammar/windows/catalog.md` (from `patterns-windows.ts`) with its own components (`shared/windows.ts`, `web/windows/`); `kit.md`, `ios/catalog.md`, `paint.md`, `icons.md`, `subjects.md` are **generated** (`npm run grammar:export`), never hand-edited. An **idiom** = grammar + catalog (+ its `patterns*.ts` and its own set of components) + stylesheet, chosen per app (`src/shared/idioms.ts`, `src/server/idioms.ts`, `src/web/kit/idioms.ts`); the pipeline holds only the `Reading` and reads everything else off the grammar, so any grammar runs. Long form for humans: `docs/writing-a-grammar.md`. Rationale and open problems: `docs/grammar.md`.
...
6. If `grammar/screen.md` or a catalog changed on purpose: `RECORD=1 node --import tsx --test src/server/grammar/regression.test.ts` (rewrites `src/server/grammar/fixtures/screen.json` = readings + decided of the grammar, and `fixtures/<idiom>.json` = what each idiom draws), then `npx tsc`, then land per house rules (commit to main, push, redeploy: `PROJECT=jev2ui-3281b2 ./deploy.sh`). Verify live through `runMock` before landing, never only by tests.
7. Never edit generated files. To add a pattern/source: edit `src/server/grammar/patterns.ts` / `fill.ts`, then `npm run grammar:export`. To add an idiom: **write its own grammar first** (`grammar/<idiom>/screen.md`, from the design system's own guidelines: its kinds, its questions, its rules, examples labelled before the first run) — an idiom is a different decision graph; a catalog + stylesheet over `screen.md` is a rendering, not an idiom, and was rejected as such. Then `src/shared/idioms.ts` (name, catalogId, stylesheets, family), `src/server/idioms.ts` (grammar, catalog file, patterns), **its own set of components**: schemas + refs as a `ComponentSet` in `src/shared/<idiom>.ts` registered in `src/shared/sets.ts`, and a `Drawing` (a function per component, `this` = the `Surface`: `value`, `words`, `has`, `node`, `kids`, `tap`, `flip`, `icon`, `baked`) registered in `src/web/sets.ts` — never add components to `KIT`; `src/web/kit/idioms.ts` (bundle the sheet; `surface.css` is always layered first), `export.ts` (write its catalog), then export + RECORD. Its frame pattern may have knobs of its own (`patterns-ios.ts`: presentation, title, leading, trailing) and says `over(knobs)` when it sits over the screen it came from (the browser is told in a `frame` event); the pipeline holds only the reading, so the grammar may ask anything.
...
| `## id (t1, t2)` | Traits. Part: `never padding`. Question: `app` = the app's, not the screen's: asked of its first screen, then given to every screen after (browser carries `journey.settled`); `given` = never asked; the runner supplies the value via `readGrammar(..., {values})` (a given noul nobody gives is no, so phrase it as the exception: screen.md's `no_photographs`, `no_symbols`, `no_cards` come from the design). Later question: `once written` \| `of each X in LIST` \| `of each X in LIST with FIELD` \| `within each Y in OUTER` \| `among each X in LIST`. |
...
Chains: `from library by <choice id> else painted else placeholder`. Sources (kit.md ▸ Sources): `set` (shelf, library: Jev picks or "none"; `by` names the shelf), `maker` (baked, painted, written), `terminal` (closed, placeholder), plus `decided`, `computed` (worked out by the pattern: `Pattern.computed[slot](elements)`, applied when the part is complete and the field's `when` holds; `details.strong` = last row). Chain must end in a terminal or a `/path`. `fresh` skips sets. A picture field with no `by` is a portrait if its slot is `portrait`, else the words decide.
...
`loadGrammar(fileOrName)` (resolves links); `parseGrammar/printGrammar/checkGrammar`; `questionsOf(g)` → Jev `Questions`; `readGrammar(g, answers, JEV, {among?, blocks?, values?})` → `Reading {kind, blocks, values, p, decisions}`; `yieldOf(g, id, value)`; `schemaOf(node, reading)` → writer JSON schema or null; `treeOf(patterns, g, node, reading, look)` → kit components; `frameOf(patterns, g, reading, look, parts:{name,root}[])`; `boundIn(components)` → bound paths; `partsOf(g, reading)`; `checkBindings(g, catalog)`; `fill(chain, fillers, sourcesOf(kit), {fresh})`; `decide(g, part, content, {description, reading, calibration, needed, known})` → `Asked[] {outer?, state, questions, read(answers)→{decorations, decisions}}`; `decorate(content, decorations)`. Also: `givensOf(g)`, `isGiven(node)`, `EMPTY` reading, `holdsIn(reading, atom)`, `appliesIn(g, node, reading)` (scope), `contentNodes(g)`, `wholeOf(node, holds)`, `framePatternOf(catalog, g)`, `frameKnobsOf(g, reading, knobs)`, `kindsOf(g)`. Tool (`src/server/idioms.ts`): `IDIOMS[id]` = `Idiom {id, name, graph, patterns, catalogId}`; `idiom()` = the request's (X-Idiom header, AsyncLocalStorage; the kit's outside a request); `Graph` (`mock/graph.ts`) has `grammar`, `catalog`, `sources`, `kinds`, `blocks`, `partNode`, `optionsOf`, `toldOf`, `chainOf(part[, field])`, `ratioOf`, `keptOf(sent, blocks)` (what stays of a sent reading; reads an old `ScreenPlan` too). `customOf(graph, node, reading)` (bake.ts) → `Custom {part, contract, told, ratio, kind, others, itemSchema?, chain}`. There is no `ScreenPlan`: the pipeline holds a `Reading`, the browser holds it as `screen.plan` and sends it back as `edit.plan`. Pipeline: `src/server/mock/pipeline.ts` (`runMock`): hooks per part come from the part (`hooksOf`): element picture fields, later questions, `form`, `computed`. Every once-written hook must be wrapped in `once`/`asTheyComplete`: a refresh re-fires hooks (a missing `once` looped ~9k Jev calls once).
...
- iOS frame (iOS's own set, `src/shared/ios.ts`: the kit's with its own `Screen` and `AppBar`): `AppBar.trailing` (a word), `leading: cancel`, `Screen.presentation: sheet|actionsheet|fullscreen`; kind traits like `presentation sheet, leading cancel, trailing done, title large`. HIG button order is a writer note + CSS `:has`, not a rule. Which button only closes what it is on (Cancel) is decided: `actions` slot `closes`, `decided by dismisses` in both grammars; `Button.closes` true ⇒ the kit taps it as `back`. The browser guesses nothing from labels, so a new grammar with dialogs must decide `closes` or its Cancel leads somewhere.
- A frame says what its slots are for with `roles: { title, destinations, main }` (make.ts): what fills the title slot is written first, the app map's destinations land in what fills the destinations slot (fields must still be named `items`/`label`/`icon`/`active`, the app map's shape) wherever the frame draws them, capped at that list's max; `main` is the knob a main screen turns. Still the tool's: the IA's activity table knows the kit's kinds; `BAKED.contract` is typed in the kit's words (`customOf` maps to the nearest); the design's givens reach only a grammar that names `no_photographs` etc. `npm run probe:share` counts what an idiom shares.
- Only one idiom's stylesheet is on the page at a time (the kit and iOS share class names). A tap is the kit's protocol (kinds `item`/`row`/`nav`/`appbar`/`action`/`submit`/`back`/`part`, `source` from a data path, `group`): another set's components must tap in it to be followed (docs/grammar.md item 64). An app keeps the idiom it was made in; there is no switch over an app's screens (the grammar is a preset in the bar that new apps are made in), so never write a grammar to be compatible with another.
