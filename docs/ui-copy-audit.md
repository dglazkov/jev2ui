...
| # | Now | Proposed |
|---|---|---|
| A1 | `A new app` (app with no title yet) | `Untitled app` |
| A2 | `Anyone with the link can open it. Make it yours alone again.` | `Anyone with the link can open this app. Select to make it private.` |
| A3 | `Only you can open it. Let anyone with the link.` | `Only you can open this app. Select to share it with a link.` |
| A4 | `Anyone with the link can open it` (snackbar) | `Anyone with the link can now open this app.` |
| A5 | `Only you can open it now` (snackbar) | `Only you can open this app now.` |
| A6 | `by {owner}` / fallback `someone` | `Shared by {owner}` / `Shared by another user` |
| A7 | `Keep this app, every screen and turn of it, to open again` (Save tooltip) | `Save this app, with every screen and message, so that you can open it later.` |
| A8 | `Save it, and let anyone with the link open it: they need not sign in` (Share tooltip) | `Save this app and create a link. Anyone with the link can open it without signing in.` |
...
| C16 | `Started afresh` (snackbar) | `Started a new app` |
...
| # | Now | Proposed |
|---|---|---|
| D1 | `{n} screens, made by {owner}. Tap through it: every screen that was made is here, and above is how it was made.` | `This app has {n} screens, created by {owner}. Tap through the preview: every screen that was generated is included. To see how a screen was generated, select the info icon.` |
| D2 | `Making and changing things takes a name, and a place on the list.` | `To create or change apps, sign in with an account that's on the access list.` |
| D3 | `You are signed in as {email}, which is not on the list of people who can make things here.` | `You're signed in as {email}, which isn't on the access list. To create or change apps, ask an admin to add your address.` |
...
| # | Now | Proposed |
|---|---|---|
| F1 | `{n} apps · every screen and turn of each` | `{n} apps, with every screen and message` |
| F2 | `describe it, or start from an example` (New app tile) | `Describe an app, or start from an example.` |
...
| F6 | `Only you can open it now` (snackbar) | `Only you can open this app now.` |
| F7 | `Anyone with the link can open it. Link copied` | `Link copied. Anyone with the link can open this app.` |
| F8 | `Deleted “the app”` (fallback when untitled) | `Deleted “Untitled app”` |
| F9 | `Nothing saved yet. Save keeps an app, every screen and turn of it; Share also lets anyone with the link open it.` | `You haven't saved any apps yet. Save keeps an app with all of its screens and messages. Share also creates a link that anyone can open.` |
...
| H2 | `Chat` / `Preview` / `Design` / `Library` / `Settings` / `New app` | Unchanged. |
...
| L4 | role tooltip `makes things, so many a day` | `Can create apps, up to a daily run limit.` |
| L5 | role tooltip `makes things, and edits this list` | `Can create apps and edit the access list.` |
| L6 | role tooltip `is kept out, whatever a wider line says` | `Can't create apps, even if a broader pattern allows it.` |
...
| N7 | `nothing is saved here: nobody has to sign in, so nothing would be anybody's` | `This server doesn't save apps, because it doesn't require sign-in.` |
| N8 | `there is no such app, or it is not shared` | `That app doesn't exist, or it isn't shared.` |
| N9 | `saving is for the people who can make things here` | `To save an app, your address must be on the access list.` |
| N10 | `this cannot be saved: {why}` | `Can't save this app: {why}` |
| N11 | `there is no such app of yours` | `You don't have an app with that ID.` |
...
| N22 | `its screens do not add up` | `The app's screens don't match its screen list.` |
| N23 | `the component on "{title}" is not what it says it is` | `The custom component on “{title}” doesn't match its definition.` |
| N24 | `its DESIGN.md is too long to save` / `one of its screens is too large to save` | `The app's DESIGN.md is too large to save.` / `One of the app's screens is too large to save.` |
