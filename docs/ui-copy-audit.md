...
| E9 | (2026-09-23; replaces the idiom control, which is gone from the design panel) the grammar, a step of the path in the bar: the grammar's name (`Kit`, `iOS`); its title `New apparitions are made as an iOS app. To use another grammar, select it here.`, over an app `Habit Tracker is an iOS app and stays one. To make new apparitions with another grammar, select it here.`, or over an app made in another grammar than the preset `Habit Tracker was made as a Kit app and stays one. New apparitions are made as an iOS app.` Its menu (aria-label `Grammar`): each grammar with a line (`The tool's own components`, `Apple · iPhone`); over an app, the note `Habit Tracker stays an iOS app. Choosing another grammar starts a new apparition.` and `To keep this one, save it first.` or `This one stays in your library.` The empty state `What iOS app do you want to make?` and the box `Describe an iOS app, or one screen of it`. The library's filter (aria-label `Grammar`, `All`) and new tile `New iOS apparition` / `Describe an iOS app, or start from an example.` | Written to the guide from the start: present tense, "the tool" for the kit's grammar, no internal names (`kit` is the id; `Kit` is what it is called). "Grammar" is the word people see; "idiom" stays the code's. |
